...
Job_Titles <-  Job_Titles %>% str_replace_all("actuarial", "acturial")
...
DA <- Job_Titles[str_detect(Job_Titles, "anal|statist|acturial|research|statician")] %>% unique()
...
Bachelor_Jobs<-Defined_jobs[str_detect(Defined_jobs,"(b\\s*s\\s*c\\s*|b\\s*s\\s*)|(bachelor\\s*|bachelors\\s*)")]
length(Bachelor_Jobs)

non_bach<-Defined_jobs[!str_detect(Defined_jobs,"(b\\s*s\\s*c\\s*|b\\s*s\\s*)|(bachelor\\s*|bachelors\\s*)")]
length(non_bach)
#Finding jobs with minimum educational qualification is a MSc
Master_Jobs<-non_bach[str_detect(non_bach,"(master)|(m\\s*s)|(masters)|(post\\s*graduate)")]
length(Master_Jobs)

#finding other category

non_msc<-non_bach[!str_detect(non_bach,"(master)|(m\\s*s)|(masters)|(post\\s*graduate)")]
length(non_msc)

phd_jobs<-non_msc[str_detect(non_msc,"(p\\s*h\\s*d)|(ph)")]
length(phd_jobs)



non_phd<-non_msc[!str_detect(non_msc,"(p\\s*h\\s*d)|(p\\s*h)")]
length(non_phd)
...
write_csv(DSraw, "Wrangled_data.csv")
